...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Falha interna no processamento do webhook." }, { status: 500 });
  }
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Falha ao processar retries/worker." }, { status: 500 });
  }
}
